# -*- coding: utf-8 -*-
"""
Created on Sat May  2 16:48:09 2020
...
class ChatBot:
  ...
  #define .chat() below:
  def chat(self):
    user_message=input('Hey!! What\'s your name? ')
  ...
  def find_intent_match(self,responses,user_message):
    ...
    similarity_list=[compare_overlap(response,user_message) for response in responses]
  ...
  def find_entities(self,user_message):
    ...
    word2vec_result=computer_similarity(tokens,category)
    word2vec_result.sort(key=lambda x: x[2])
    if len(word2vec_result)==0:
      return blank_spot
    else:
      return word2vec_result[0]
  #define .respond() below:
  def respond(self,user_message):
    best_response=self.find_intent_match(responses,user_message)
    entity=self.find_entities(user_message)
    print(best_response.format(entity))
    input_message=input("Any other query? ")
